// forked from design-sync lib/source-kit.mjs — exclude app-bootstrap files (createRoot/render side-effects) from the synth entry
...
export async function resolvePackage(ctx) {
  ...
  if (!entry) {
    ...
    // Exclude app-bootstrap modules (createRoot/hydrateRoot/ReactDOM.render):
    // they export nothing and run render side-effects at import, which crash the
    // bundle IIFE (no #root) and poison window.<global>. See NOTES.md.
    const BOOTSTRAP_RX = /createRoot\s*\(|hydrateRoot\s*\(|ReactDOM\.render\s*\(/;
