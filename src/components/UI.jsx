...
export function Btn({ children, onClick, variant = "primary", size = "md", full, disabled, style: extra }) {
  ...
  const variants = {
    primary: { background: `linear-gradient(135deg, ${T.blood}, #9C3050)`, color: "#F6EFE6", border: "none" },
    champagne: { background: `linear-gradient(135deg, ${T.champagne}, ${T.champSoft})`, color: "#1a0a0f", border: "none" },
    ghost: { background: "transparent", color: T.champagne, border: `1px solid rgba(215,185,130,0.25)` },
    soft: { background: "rgba(31,12,24,0.8)", color: T.textSecondary, border: `1px solid rgba(215,185,130,0.14)` },
...
export function Card({ children, style, premium, onClick, hover }) {
  return (
    <div onClick={onClick} style={{
      ...
      onMouseEnter={e => (hover || onClick) && (e.currentTarget.style.borderColor = "rgba(215,185,130,0.28)", e.currentTarget.style.boxShadow = "0 0 40px rgba(185,130,142,0.14)")}
      onMouseLeave={e => (hover || onClick) && (e.currentTarget.style.borderColor = "rgba(215,185,130,0.14)", e.currentTarget.style.boxShadow = T.glow)}
    >{children}</div>
  );
}
...
export function Pill({ children, color, style }) {
  const colors = {
    rose: { bg: "rgba(185,130,142,0.15)", text: "#D4A0AA" },
    champagne: { bg: "rgba(215,185,130,0.14)", text: T.champagne },
    success: { bg: "rgba(141,175,122,0.15)", text: T.success },
    danger: { bg: "rgba(184,92,104,0.15)", text: T.danger },
    muted: { bg: "rgba(58,38,48,0.6)", text: T.textMuted },
    blood: { bg: "rgba(124,38,63,0.25)", text: "#D4809A" },
  };
...
export function ProgressBar({ value, max, color, height = 5 }) {
  ...
  return (
    <div style={{ height, background: "rgba(255,255,255,0.06)", borderRadius: height, overflow: "hidden" }}>
      <div style={{ width: `${pct}%`, height: "100%", background: color || `linear-gradient(90deg, ${T.blood}, ${T.rose})`, borderRadius: height, transition: "width 0.6s ease" }} />
    </div>
  );
}
