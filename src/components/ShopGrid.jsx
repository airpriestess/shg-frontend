  ...
  return (
    <button onClick={async () => { setBusy(p.name); await buyProduct(p); setBusy(""); }}
      ...
      <div style={{ padding:"10px 12px 12px", display:"flex", flexDirection:"column", gap:8, flex:1, textAlign:"center" }}>
        <div className="shg-shop-label" style={{ fontSize:13, fontWeight:300, letterSpacing:0, textTransform:"none" }}>{p.name}</div>
        <div style={{ display:"flex", alignItems:"center", justifyContent:"center", gap:10, marginTop:"auto" }}>
          {p.price && <span className="shg-shop-label" style={{ fontSize:13, fontWeight:300, letterSpacing:0 }}>{p.price}</span>}
          <span style={{ padding:"5px 12px", background:G, borderRadius:999, color:"#000", fontSize:12, fontWeight:300 }}>{busy===p.name ? "Opening…" : p.price==="Free" ? "Get it" : p.price ? "Buy" : "View"}</span>
        </div>
      </div>
    </button>
  );
