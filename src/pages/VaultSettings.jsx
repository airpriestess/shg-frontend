...
export default function VaultSettings({ userTier, onSignOut, onUpgrade }) {
  ...
  const Row = ({ label, value }) => (
    <div style={{ display: "flex", justifyContent: "space-between", padding: "11px 0", borderBottom: "1px solid #1e1a08" }}>
    ...
    <div style={{ padding: "28px 24px", overflowY: "auto", height: "100%", maxWidth: 660, margin: "0 auto" }} className="mob-pb fade">
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
        ...
        <div style={{ background: "#0a0900", border: "1px solid #1e1a08", borderRadius: 14, padding: "22px 22px" }}>
          <div style={{ fontSize: 11, color: RG, fontWeight: 700, letterSpacing: "0.16em", textTransform: "uppercase", marginBottom: 16 }}>Account</div>
        ...
        {/* ── STORAGE ── */}
        <div style={{ background: "#0a0900", border: "1px solid #1e1a08", borderRadius: 14, padding: "22px 22px" }}>
          <div style={{ fontSize: 11, color: RG, fontWeight: 700, letterSpacing: "0.16em", textTransform: "uppercase", marginBottom: 16 }}>Evidence Vault Storage</div>
          <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 8 }}>
            <span style={{ fontSize: 15, color: T.textPrimary, fontWeight: 600 }}>Used</span>
            <span style={{ fontSize: 14, color: RG, fontWeight: 700 }}>{USER.storageUsedMb} MB <span style={{ color: T.textMuted, fontWeight: 400 }}>of {limit.toLocaleString()} MB</span></span>
          </div>
          <div style={{ height: 6, background: "#1e1a08", borderRadius: 3, marginBottom: 12 }}>
        ...
        {/* ── LISTENING REMINDERS ── */}
        <div style={{ background: "#0a0900", border: "1px solid #1e1a08", borderRadius: 14, padding: "22px 22px" }}>
          <div style={{ fontSize: 11, color: RG, fontWeight: 700, letterSpacing: "0.16em", textTransform: "uppercase", marginBottom: 16 }}>Listening Reminders</div>
          ...
          <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between" }}>
            ...
            {notifStatus === "denied" ? (
              ...
              <button onClick={toggleNotifications} style={{
                ...
                background: remindersOn ? G : "#1e1a08",
        ...
        {/* ── HOW TO LISTEN ── */}
        <div style={{ background: "#0a0900", border: "1px solid #1e1a08", borderRadius: 14, padding: "22px 22px" }}>
          <div style={{ fontSize: 11, color: RG, fontWeight: 700, letterSpacing: "0.16em", textTransform: "uppercase", marginBottom: 16 }}>How to Listen</div>
          <div style={{ display: "flex", flexDirection: "column", gap: 0 }}>
            {[
              ...
              <div key={i} style={{ padding: "14px 0", borderBottom: i < arr.length-1 ? "1px solid #1e1a08" : "none" }}>
                <div style={{ fontSize: 13, fontWeight: 700, color: RG, marginBottom: 5 }}>{g.t}</div>
        ...
        {/* ── INSTALL ── */}
        <div style={{ background: "#0a0900", border: "1px solid #1e1a08", borderRadius: 14, padding: "22px 22px" }}>
          <div style={{ fontSize: 11, color: RG, fontWeight: 700, letterSpacing: "0.16em", textTransform: "uppercase", marginBottom: 14 }}>Install the App</div>
        ...
        <div style={{ display: "flex", gap: 10 }}>
          {[["YouTube", "https://www.youtube.com/@Reshma.Oracle"], ["Instagram", "https://www.instagram.com/reshma.oracle/"]].map(([l, u]) => (
            <a key={l} href={u} target="_blank" rel="noopener noreferrer"
              style={{ flex: 1, padding: "14px 0", background: "#0a0900", border: "1px solid #1e1a08", borderRadius: 12, color: T.textMuted, fontSize: 14, textAlign: "center", textDecoration: "none" }}>
              {l} ↗
            </a>
          ))}
        </div>
