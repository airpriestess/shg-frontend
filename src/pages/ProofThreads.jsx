  ...
  return (
    <div style={{ padding: "28px 24px", maxWidth: 880, overflowY: "auto", height: "100%" }} className="mob-pb">
      ...
      <div style={{ marginBottom: 28 }}>
        <h1 style={{ fontSize: "clamp(22px,2.5vw,28px)", fontWeight: 300, color: "#fdf0e8", marginBottom: 6, letterSpacing: "0.01em" }}>Proof Threads</h1>
        ...
        {(localMoodBefore !== t.mood_before || localMoodAfter !== t.mood_after) && (
          <div style={{ marginTop: 10, display: "flex", gap: 8 }}>
            <button onClick={() => { onSaveMood("mood_before", localMoodBefore); onSaveMood("mood_after", localMoodAfter); }}
              style={{ padding: "7px 16px", background: "linear-gradient(90deg,#F5E0A0 0%,#E8B870 22%,#BFA5D8 52%,#2CB7A7 78%,#167A6B 100%)", border: "none", borderRadius: 8, color: "#000", fontSize: 13, fontWeight: 700, cursor: "pointer" }}>Save</button>
          ...
          {(t.entries || []).map((e, idx) => (
            <div key={e.id} style={{ display: "flex", gap: 0 }}>
              ...
              <div style={{ display: "flex", flexDirection: "column", alignItems: "center", marginRight: 16, flexShrink: 0 }}>
                <div style={{ width: 36, height: 36, borderRadius: "50%", background: e.isFinal ? "linear-gradient(135deg, " + T.success + ", " + T.gold + ")" : "#0f0b01", border: `2px solid ${e.isFinal ? T.success : "#fdf0e8"}`, display: "flex", alignItems: "center", justifyContent: "center", fontSize: 14, flexShrink: 0, zIndex: 1 }}>{TYPE_ICON[e.type] || "·"}</div>
              ...
              <Card onClick={() => setExpandedEntry(expandedEntry === e.id ? null : e.id)} style={{ flex: 1, padding: "14px 16px", marginBottom: 10, cursor: "pointer", background: e.isFinal ? "linear-gradient(135deg, rgba(141,175,122,0.1), #140f04)" : "#060410", border: e.isFinal ? "1px solid rgba(141,175,122,0.25)" : "1px solid #1e1c0a" }}>
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", gap: 8, marginBottom: 6 }}>
                  <div style={{ flex: 1 }}>
                    <div style={{ display: "flex", gap: 8, alignItems: "center", marginBottom: 4 }}>
                      ...
                      <span style={{ fontSize: 11, color: "#fdf0e8" }}>Day {e.dayNumber}</span>
                    </div>
                    <div style={{ fontSize: 15, fontWeight: 600, color: "#e8e0d0", lineHeight: 1.35 }}>{e.title}</div>
                  </div>
                  <span style={{ fontSize: 12, color: "#fdf0e8", flexShrink: 0 }}>{e.happenedAt}</span>
                </div>
                ...
                {e.type === "Voice Proof" && (
                  <div style={{ display: "flex", alignItems: "center", gap: 10, marginTop: 8 }}>
                    ...
                    <span style={{ fontSize: 11, color: "#fdf0e8" }}>{e.durationSec}s</span>
                  </div>
                )}
  ...
  return (
    <div>
      <div style={{ border: "2px dashed #2CB7A7", borderRadius: 12, padding: "28px 20px", textAlign: "center", marginBottom: 16, cursor: "pointer", background: "#040208" }}
        ...
        {file ? <div style={{ color: T.success, fontSize: 14, fontWeight: 600 }}>📷 {file.name}</div> : <div><div style={{ fontSize: 28, marginBottom: 8 }}>📷</div><div style={{ fontSize: 14, color: T.textMuted }}>Tap to upload photo proof</div><div style={{ fontSize: 12, color: "#fdf0e8", marginTop: 4 }}>JPG · PNG · WEBP · HEIC</div></div>}
      </div>
