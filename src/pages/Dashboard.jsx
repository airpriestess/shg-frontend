...
function StatCard({ value, label }) {
  return (
    <div style={{ background: "rgba(255,255,255,0.85)", border: "1px solid rgba(183,110,121,0.15)", borderRadius: 14, padding: "20px 16px", textAlign: "center" }}>
...
function Tabs({ tabs, active, onChange }) {
  return (
    <div style={{ display: "flex", gap: 0, borderBottom: "1px solid rgba(183,110,121,0.15)", marginBottom: 20 }}>
      {tabs.map(t => (
...
export default function Dashboard({ userTier, onNavigate, onAddProof, onCreateThread }) {
  ...
  return (
    <div style={{ height: "100%", overflowY: "auto", background: "linear-gradient(160deg,#fef4f8 0%,#fde8f4 30%,#f8eefa 60%,#fef4f0 100%)", color: "#1a0808" }} className="mob-pb">
        ...
        {activeTab === "Overview" && (
          <>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 14, marginBottom: 24 }} className="grid-2">
              ...
              {/* Current Ritual */}
              <div style={{ background: "rgba(255,255,255,0.85)", border: "1px solid rgba(183,110,121,0.15)", borderRadius: 16, padding: "20px" }}>
            ...
            <div style={{ marginBottom: 24 }}>
              ...
              <div style={{ display: "grid", gridTemplateColumns: "repeat(3,1fr)", gap: 10 }} className="grid-3">
                {RECENT_PROOF.map((p, i) => (
                  <div key={i} onClick={() => onNavigate("proof-threads")} style={{ background: "rgba(255,255,255,0.85)", border: "1px solid rgba(183,110,121,0.15)", borderRadius: 12, overflow: "hidden", cursor: "pointer", transition: "border-color 0.15s" }}
                    onMouseEnter={e => e.currentTarget.style.borderColor = RG + "44"}
                    onMouseLeave={e => e.currentTarget.style.borderColor = "#1c1828"}
                  >
                    <div style={{ height: 68, background: `${RG}0e`, display: "flex", alignItems: "center", justifyContent: "center", fontSize: 26, borderBottom: "1px solid rgba(183,110,121,0.15)" }}>{p.icon}</div>
            ...
            {manifested.length > 0 && (
              <div style={{ marginBottom: 24 }}>
                ...
                <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
                  {manifested.map(t => (
                    <div key={t.id} style={{ background: "rgba(255,255,255,0.85)", border: "1px solid #1a3a1a", borderRadius: 12, padding: "14px 16px", display: "flex", justifyContent: "space-between", alignItems: "center", gap: 12 }}>
            ...
            {/* ── STORAGE ── */}
            <div style={{ background: "rgba(255,255,255,0.85)", border: "1px solid rgba(183,110,121,0.15)", borderRadius: 14, padding: "16px 18px" }}>
        ...
        {activeTab === "Proof Threads" && (
          <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
            {PROOF_THREADS.map(t => {
              ...
              return (
                <div key={t.id} onClick={() => onNavigate("proof-threads")} style={{ background: "rgba(255,255,255,0.85)", border: "1px solid rgba(183,110,121,0.15)", borderRadius: 14, padding: "18px 20px",
                ...
                >
                  ...
                  {t.mood_before && t.mood_after && (
                    <div style={{ display: "flex", gap: 6, alignItems: "center", marginBottom: 12 }}>
                      <span style={{ fontSize: 11, padding: "2px 8px", background: "rgba(255,255,255,0.85)", border: "1px solid #222", borderRadius: 20, color: "#5a3838" }}>{t.mood_before}</span>
                ...
                {TIMELINE.map((item, i) => {
                  ...
                  return (
                    <div key={i} style={{ display: "flex", gap: 16 }}>
                      ...
                      <div style={{ flex: 1, background: "rgba(255,255,255,0.85)", border: `1px solid ${col}22`, borderRadius: 12, padding: "14px 16px", marginBottom: 8 }}>
