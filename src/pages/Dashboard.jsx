...
const STATUS_LABEL = {
  ...
  "Paused": { label: "Paused", color: "#b09888" },
};
...
function StatCard({ value, label }) {
  return (
    <div style={{ background: "#06040c", border: "1px solid #1c1828", borderRadius: 14, padding: "20px 16px", textAlign: "center" }}>
      <div style={{ fontSize: 28, fontWeight: 800, color: "#B76E79", lineHeight: 1, marginBottom: 6, fontFamily: "'Jost',sans-serif" }}>{value}</div>
      <div style={{ fontSize: 11, color: "#b09888", letterSpacing: "0.1em", textTransform: "uppercase" }}>{label}</div>
    </div>
  );
}
...
function Tabs({ tabs, active, onChange }) {
  return (
    <div style={{ display: "flex", gap: 0, borderBottom: "1px solid #1c1828", marginBottom: 20 }}>
      {tabs.map(t => (
...
export default function Dashboard({ userTier, onNavigate, onAddProof, onCreateThread }) {
  ...
  return (
    <div style={{ height: "100%", overflowY: "auto", background: "#000", color: "#f2ece4" }} className="mob-pb">
      <div style={{ maxWidth: 880, margin: "0 auto", padding: "28px 24px" }}>
        ...
        {activeTab === "Overview" && (
          <>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 14, marginBottom: 24 }} className="grid-2">
              ...
              {/* Current Ritual */}
              <div style={{ background: "#06040c", border: "1px solid #1c1828", borderRadius: 16, padding: "20px" }}>
                  ...
                  <>
                    <div style={{ display: "flex", gap: 14, alignItems: "center", marginBottom: 14 }}>
                      ...
                      <div style={{ minWidth: 0 }}>
                        <div style={{ fontSize: 14, fontWeight: 700, color: "#f2ece4", marginBottom: 3, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{currentAudio.title}</div>
                    ...
                    <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                      <div style={{ display: "flex", gap: 10 }}>
                        <span style={{ fontSize: 12, color: "#b09888" }}>🔥 {USER.listeningStreak}d streak</span>
                        <span style={{ fontSize: 12, color: "#b09888" }}>{currentAudio.lastListenedAt}</span>
                      </div>
                      <Btn size="sm" variant="primary" onClick={() => onNavigate("audio-vault")}>▶ Resume</Btn>
                    </div>
                  </>
                ) : (
                  <div style={{ fontSize: 13, color: "#b09888", padding: "20px 0" }}>No active ritual. Start an audio to begin.</div>
                )}
              </div>
            ...
            <div style={{ marginBottom: 24 }}>
              ...
              <div style={{ display: "grid", gridTemplateColumns: "repeat(3,1fr)", gap: 10 }} className="grid-3">
                {RECENT_PROOF.map((p, i) => (
                  <div key={i} onClick={() => onNavigate("proof-threads")} style={{ background: "#06040c", border: "1px solid #1c1828", borderRadius: 12, overflow: "hidden", cursor: "pointer", transition: "border-color 0.15s" }}
                    onMouseEnter={e => e.currentTarget.style.borderColor = RG + "44"}
                    onMouseLeave={e => e.currentTarget.style.borderColor = "#1c1828"}
                  >
                    <div style={{ height: 68, background: `${RG}0e`, display: "flex", alignItems: "center", justifyContent: "center", fontSize: 26, borderBottom: "1px solid #1c1828" }}>{p.icon}</div>
                    <div style={{ padding: "10px 12px" }}>
                      <div style={{ fontSize: 12, fontWeight: 300, color: "#f2ece4", marginBottom: 2 }}>{p.label}</div>
                      <div style={{ fontSize: 11, color: "#b09888", marginBottom: 2, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{p.thread}</div>
            ...
            {manifested.length > 0 && (
              <div style={{ marginBottom: 24 }}>
                ...
                <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
                  {manifested.map(t => (
                    <div key={t.id} style={{ background: "#06040c", border: "1px solid #1a3a1a", borderRadius: 12, padding: "14px 16px", display: "flex", justifyContent: "space-between", alignItems: "center", gap: 12 }}>
                      <div style={{ flex: 1, minWidth: 0 }}>
                        <div style={{ fontSize: 14, fontWeight: 600, color: "#f2ece4", marginBottom: 3, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{t.intentionTitle}</div>
                        <div style={{ display: "flex", gap: 10, alignItems: "center" }}>
                          <span style={{ fontSize: 12, color: "#b09888" }}>🎧 {t.linkedAudioTitle}</span>
                          <span style={{ fontSize: 12, color: "#b09888" }}>{t.daysActive}d</span>
            ...
            {/* ── STORAGE ── */}
            <div style={{ background: "#06040c", border: "1px solid #1c1828", borderRadius: 14, padding: "16px 18px" }}>
              <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 10 }}>
                <span style={{ fontSize: 12, color: "#b09888", textTransform: "uppercase", letterSpacing: "0.1em" }}>Storage</span>
              ...
              <div style={{ display: "flex", gap: 16 }}>
                <span style={{ fontSize: 11, color: "#b09888" }}>📷 {PROOF_THREADS.reduce((s,t) => s+(t.photoProofCount||0),0)} photos</span>
                <span style={{ fontSize: 11, color: "#b09888" }}>🎙 {PROOF_THREADS.reduce((s,t) => s+(t.voiceProofCount||0),0)} voice notes</span>
              </div>
            </div>
          </>
        )}
        ...
        {activeTab === "Proof Threads" && (
          <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
            {PROOF_THREADS.map(t => {
              const sc = STATUS_LABEL[t.status] || { label: t.status, color: "#b09888" };
              return (
                <div key={t.id} onClick={() => onNavigate("proof-threads")} style={{ background: "#06040c", border: "1px solid #1c1828", borderRadius: 14, padding: "18px 20px",
                ...
                >
                  <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", gap: 12, marginBottom: 10 }}>
                    <div style={{ flex: 1, minWidth: 0 }}>
                      <div style={{ fontSize: 15, fontWeight: 700, color: "#f2ece4", marginBottom: 4, lineHeight: 1.3 }}>{t.intentionTitle}</div>
                    ...
                    <div style={{ textAlign: "right", flexShrink: 0 }}>
                      ...
                      <div style={{ fontSize: 11, color: "#b09888" }}>{t.daysActive}d</div>
                    </div>
                  </div>
                  {/* Photo thumbnails */}
                  <div style={{ display: "flex", gap: 6, alignItems: "center", marginBottom: 12 }}>
                    ...
                    {(t.photoProofCount||0) > 4 && <span style={{ fontSize: 11, color: "#b09888" }}>+{t.photoProofCount-4}</span>}
                    {(t.voiceProofCount||0) > 0 && <span style={{ fontSize: 12, color: "#b09888", marginLeft: 4 }}>🎙 {t.voiceProofCount}</span>}
                    {(t.signCount||0) > 0 && <span style={{ fontSize: 12, color: "#b09888" }}>◈ {t.signCount}</span>}
                  </div>
                  {t.mood_before && t.mood_after && (
                    <div style={{ display: "flex", gap: 6, alignItems: "center", marginBottom: 12 }}>
                      <span style={{ fontSize: 11, padding: "2px 8px", background: "#06040c", border: "1px solid #222", borderRadius: 20, color: "#b09888" }}>{t.mood_before}</span>
        ...
        {activeTab === "Timeline" && (
          <div>
            <div style={{ fontSize: 13, color: "#b09888", marginBottom: 24, lineHeight: 1.7 }}>
                ...
                {TIMELINE.map((item, i) => {
                  ...
                  return (
                    <div key={i} style={{ display: "flex", gap: 16 }}>
                      ...
                      <div style={{ flex: 1, background: "#06040c", border: `1px solid ${col}22`, borderRadius: 12, padding: "14px 16px", marginBottom: 8 }}>
                        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", gap: 8 }}>
                          <div>
                            <span style={{ fontSize: 11, color: col, fontWeight: 700, letterSpacing: "0.1em", textTransform: "uppercase" }}>{item.day}</span>
                            <span style={{ fontSize: 11, color: "#b09888", marginLeft: 8 }}>{item.event}</span>
                          </div>
                          {item.photo && (
