...
/* SOUNDWAVE */
.sw{display:flex;align-items:center;gap:3px;height:40px}
...
.sw span:nth-child(8){height:12px;animation-delay:.1s}
.sw span:nth-child(9){height:22px;animation-delay:.2s}
...
@keyframes breathe{0%,100%{transform:translate(-50%,-50%) scale(1);opacity:var(--op)}50%{transform:translate(-50%,-50%) scale(1.05);opacity:calc(var(--op)*2.5)}}
...
/* GLOW */
.gold-glow{box-shadow:0 0 40px #C8892A22,0 0 80px #C8892A0a}
...
function Btn({ children, onClick, full, outline, small, disabled }) {
  return (
    <button onClick={onClick} disabled={disabled} style={{
      ...
      gap: 8, transition: "transform 0.15s, opacity 0.2s", minHeight: 48,
...
function Rings({ count = 4, color = C.gold }) {
  ...
  return (
    <div style={{ position: "absolute", inset: 0, overflow: "hidden", pointerEvents: "none" }}>
      <div style={{ position: "absolute", top: "50%", left: "50%", transform: "translate(-50%,-50%)", width: 500, height: 500, background: `radial-gradient(circle,${color}0a 0%,transparent 70%)`, borderRadius: "50%" }} />
      {sizes.slice(0, count).map((s, i) => (
        <div key={i} className="ring" style={{ width: s, height: s, borderColor: `${color}${['22','12','09','06','04'][i]}`, "--op": ops[i], animationDelay: `${i * 0.9}s` }} />
      ))}
    </div>
  );
}
...
function SoundwaveHero() {
  return (
    <div style={{ display: "flex", alignItems: "center", justifyContent: "center", gap: 3, height: 60, marginBottom: 32 }}>
      {Array.from({ length: 24 }).map((_, i) => {
        const heights = [8, 16, 28, 20, 36, 24, 40, 18, 32, 14, 38, 22, 36, 28, 18, 40, 24, 32, 16, 28, 20, 36, 12, 24];
        return (
          <div key={i} style={{
            width: 3, borderRadius: 2,
            height: heights[i],
            background: `linear-gradient(180deg,${C.gold},${C.rose})`,
            animation: `sw ${0.8 + (i % 5) * 0.15}s ease-in-out infinite`,
...
function Landing({ onJoin, onProofOS, onDemo }) {
  ...
  const cats = [
    ...
    { id: "money", label: "Moneymaxxing", color: "#b87a20", tagline: "Money finds you first. Currently right now.",
    ...
    { id: "life", label: "Lifemaxxing", color: "#9a7a5a", tagline: "Highest timeline. Parallel reality. Activated.",
    ...
    { id: "sleep", label: "Sleep Shifting", color: "#5a7ab0", tagline: "You shift while you sleep. Wake up as her.",
      tracks: ["I manifest while I sleep · Delta · 60 min","I wake up as her · Theta · 55 min","Overnight identity shift · Delta · 8 hrs","The 180 installs silently while you dream · Delta · 8 hrs"] },
  ];
  const cat = cats.find(c => c.id === activeCat);

  const oldWay = [
    { icon: "✗", text: "Scripting for hours and watching nothing change" },
    { icon: "✗", text: "Affirmations that feel hollow and fake every single morning" },
    { icon: "✗", text: "Refreshing his Instagram at 2am wondering if he's moved on" },
    { icon: "✗", text: "Picking apart your face in the mirror looking for what's wrong" },
    { icon: "✗", text: "Coach after coach telling you to 'just believe' — €3,000 later" },
    { icon: "✗", text: "Manifesting from desperation — which is why nothing lands" },
  ];
  const newWay = [
    { icon: "✦", text: "Press play. Your subconscious does the work while you rest" },
    { icon: "✦", text: "Wake up feeling different. Calmer. More certain. Like something reset" },
    { icon: "✦", text: "He texts. Money arrives. Strangers compliment your skin. Of course they do" },
    { icon: "✦", text: "You stop needing it — and that's exactly when it arrives" },
    { icon: "✦", text: "The shift was always available. You just needed the right frequency" },
    { icon: "✦", text: "Document every win in ProofOS. Watch the pattern become undeniable" },
  ];

  const steps = [
    { n: "01", icon: "🎧", title: "You listen", body: "Self hypnosis and subliminal audio tracks. Put your headphones in. Press play. That's all you do. Works while you sleep, while you rest, while you exist." },
    { n: "02", icon: "◈", title: "Your subconscious shifts", body: "The old programming dissolves. The new identity installs. Not through effort — through repetition at the frequency level. Currently right now." },
    { n: "03", icon: "✦", title: "Reality responds", body: "He texts. Money arrives unexpectedly. People ask what you changed. Your skin shifts. Your energy shifts. The parallel reality crystallises." },
    { n: "04", icon: "◉", title: "You document the proof", body: "Log every sign in ProofOS. Link it to the exact audio you were listening to. Watch the evidence build. The pattern becomes undeniable. It is done." },
  ];
  ...
  return (
    <div style={{ background: C.black, minHeight: "100vh" }}>
      ...
      <nav style={{ position: "fixed", top: 0, left: 0, right: 0, zIndex: 300, height: 58, display: "flex", alignItems: "center", justifyContent: "space-between", padding: "0 20px", background: "rgba(0,0,0,0.96)", borderBottom: `1px solid ${C.border}`, backdropFilter: "blur(20px)" }}>
        ...
        {menuOpen && (
          <div style={{ position: "absolute", top: 58, right: 20, background: "#0a0800", border: `1px solid ${C.border}`, borderRadius: 14, padding: 12, minWidth: 200, zIndex: 400 }}>
            {[
              ["Audio Library", () => setMenuOpen(false)],
              ["ProofOS ✦", () => { setMenuOpen(false); onProofOS(); }],
              ["Preview Dashboard", () => { setMenuOpen(false); onDemo(); }],
              ["YouTube ↗", () => window.open("https://www.youtube.com/@Reshma.Oracle", "_blank")],
              ["Instagram ↗", () => window.open("https://www.instagram.com/reshma.oracle/", "_blank")],
            ].map(([l, fn], i) => (
      ...
      <div style={{ position: "relative", minHeight: "100vh", display: "flex", alignItems: "center", justifyContent: "center", paddingTop: 58, overflow: "hidden" }}>
        <Rings count={5} />
        <div style={{ position: "relative", zIndex: 1, textAlign: "center", padding: "60px 20px 80px", maxWidth: 700, margin: "0 auto", width: "100%" }}>
          <SoundwaveHero />
          <div style={{ fontSize: 11, letterSpacing: "0.35em", textTransform: "uppercase", color: C.muted, marginBottom: 20, fontWeight: 600 }}>Reshma Oracle · Audio Library</div>
          <h1 className="wm" style={{ fontSize: "clamp(38px,7vw,68px)", lineHeight: 1.1, marginBottom: 24, color: C.text }}>
            No more hoping.<br />No more waiting.<br />
            <span style={{ background: `linear-gradient(90deg,${C.gold},${C.rose})`, WebkitBackgroundClip: "text", WebkitTextFillColor: "transparent" }}>Reprogram your subconscious<br />and shift your reality.</span>
          </h1>
          <p style={{ fontSize: "clamp(16px,2.5vw,19px)", color: "#8a7a5a", lineHeight: 1.85, marginBottom: 8, maxWidth: 540, margin: "0 auto 8px" }}>
            Self hypnosis and subliminal audio tracks that reprogram your subconscious mind for effortless manifestation, identity shifting, and reality maxxing.
          </p>
          <p style={{ fontSize: 15, color: C.muted, marginBottom: 14 }}>Not on YouTube. No ads. Real voice. Original frequencies.</p>
          <p style={{ fontSize: 13, color: C.dim, marginBottom: 40 }}>50+ tracks · Lovemaxxing · Moneymaxxing · Beautymaxxing · Lifemaxxing · 4 new tracks every week</p>

          {/* FREE TRACK PLAYER */}
          <div className="gold-glow" style={{ background: "#0a0800", border: `1px solid ${C.gold}55`, borderRadius: 18, padding: "22px 24px", maxWidth: 440, margin: "0 auto 36px" }}>
            ...
            <div style={{ display: "flex", alignItems: "center", gap: 14 }}>
              ...
              <div style={{ flex: 1 }}>
                <div style={{ height: 4, background: C.border, borderRadius: 2, marginBottom: 6, cursor: "pointer", overflow: "hidden" }}
                  onClick={e => { const r = e.currentTarget.getBoundingClientRect(); const pct = (e.clientX - r.left) / r.width; if (audioRef.current && audioRef.current.duration) audioRef.current.currentTime = pct * audioRef.current.duration; }}>
                  <div style={{ width: `${progress}%`, height: "100%", background: `linear-gradient(90deg,${C.gold},${C.rose})`, borderRadius: 2, transition: "width 0.1s" }} />
                </div>
                <div style={{ fontSize: 11, color: C.dim }}>{playing ? "Playing — works while you sleep too ✦" : "Tap to listen — free, no sign up needed"}</div>
              </div>
              {playing && <div className="sw"><span /><span /><span /><span /><span /><span /><span /><span /><span /></div>}
            </div>
          </div>
      ...
      <div style={{ padding: "70px 20px", maxWidth: 860, margin: "0 auto" }}>
        <div style={{ textAlign: "center", marginBottom: 36 }}>
          ...
          <h2 className="wm" style={{ fontSize: "clamp(28px,5vw,48px)", color: C.text, marginBottom: 10 }}>Full access. No ads. Ever.</h2>
          <p style={{ fontSize: 16, color: C.muted }}>Audio Tier for the vault. Goddess Tier for the vault + ProofOS manifestation tracker.</p>
        </div>
        ...
        <div className="price-grid" style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 14, marginBottom: 14 }}>
          {[
            { id: "audio", name: "Audio Tier", price: billing === "monthly" ? "€19.99" : "€192", period: billing === "monthly" ? "/month" : "/year", sub: billing === "annual" ? "€16/mo · 2 months free" : null,
              features: ["Full self hypnosis audio library — 50+ tracks", "All 6 desire categories", "Lovemaxxing · Moneymaxxing · Beautymaxxing", "4 new tracks every week — never on YouTube", "Loop player + sleep timer", "Plays in background while phone is locked", "No ads. Ever. No interruptions at 3am."], cta: "Join Audio Tier" },
            { id: "goddess", name: "Goddess Tier", popular: true, price: billing === "monthly" ? "€33" : "€317", period: billing === "monthly" ? "/month" : "/year", sub: billing === "annual" ? "€26.40/mo · 2 months free" : null,
              features: ["Everything in Audio Tier", "ProofOS manifestation tracker ✦ coming soon", "Log desires · link to audio · capture proof", "Watch your wins build into undeniable evidence", "Early access — 48hrs before everyone else", "Monthly ritual audio included", "Goddess community"], proofos: true, cta: "Become Goddess" },
          ].map(p => (
        ...
        {/* Founder */}
        <div style={{ position: "relative", background: "linear-gradient(135deg,#0d0900,#160b01)", border: `2px solid ${C.gold}44`, borderRadius: 18, padding: 30, overflow: "hidden" }}>
          <div style={{ position: "absolute", top: 0, right: 0, width: 300, height: 300, background: `radial-gradient(circle,${C.gold}06,transparent)`, pointerEvents: "none" }} />
          <div style={{ position: "relative", zIndex: 1 }}>
            ...
            <div className="wm" style={{ fontSize: "clamp(24px,4vw,36px)", color: C.text, marginBottom: 8 }}>Founder Lifetime</div>
            ...
            <p style={{ fontSize: 15, color: "#7a6a4a", lineHeight: 1.75, marginBottom: 20, maxWidth: 560 }}>Full vault + ProofOS + every future feature + 1 GB evidence vault — forever. No subscription. No price increase. Ever. The €500 price closes permanently once the first 1,000 Founders join.</p>
        ...
        <div style={{ textAlign: "center", marginTop: 16, fontSize: 13, color: C.dim, lineHeight: 1.9 }}>No refunds after 14 days · Cancel before renewal · Web app · iPhone: Add to Home Screen · Plays in background</div>
      </div>

      {/* OLD WAY VS NEW WAY */}
      <div style={{ padding: "70px 20px", maxWidth: 900, margin: "0 auto" }}>
        <div style={{ textAlign: "center", marginBottom: 48 }}>
          <div style={{ fontSize: 11, color: C.muted, letterSpacing: "0.25em", textTransform: "uppercase", marginBottom: 12 }}>The shift</div>
          <h2 className="wm" style={{ fontSize: "clamp(28px,5vw,48px)", color: C.text }}>You already know which one you're living.</h2>
        </div>
        <div className="grid-2" style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 2 }}>
          ...
          <div style={{ background: "#0a0505", border: `1px solid #2a1010`, borderRadius: "16px 0 0 16px", padding: 28 }}>
            <div style={{ fontSize: 13, color: "#6a3a3a", fontWeight: 800, letterSpacing: "0.2em", textTransform: "uppercase", marginBottom: 20 }}>The old way</div>
            {oldWay.map((item, i) => (
              <div key={i} style={{ display: "flex", gap: 12, marginBottom: 16, alignItems: "flex-start" }}>
                <span style={{ color: "#5a2a2a", fontSize: 16, fontWeight: 700, flexShrink: 0, marginTop: 1 }}>{item.icon}</span>
                <span style={{ fontSize: 15, color: "#6a4a4a", lineHeight: 1.6 }}>{item.text}</span>
              </div>
            ))}
          </div>
          {/* New Way */}
          <div style={{ background: "#08090a", border: `1px solid ${C.gold}33`, borderRadius: "0 16px 16px 0", padding: 28 }}>
            <div style={{ fontSize: 13, color: C.gold, fontWeight: 800, letterSpacing: "0.2em", textTransform: "uppercase", marginBottom: 20 }}>The new way</div>
            {newWay.map((item, i) => (
              <div key={i} style={{ display: "flex", gap: 12, marginBottom: 16, alignItems: "flex-start" }}>
                <span style={{ color: C.gold, fontSize: 14, flexShrink: 0, marginTop: 2 }}>{item.icon}</span>
                <span style={{ fontSize: 15, color: C.text2, lineHeight: 1.6 }}>{item.text}</span>
              </div>
            ))}
          </div>
        </div>
        <div style={{ textAlign: "center", marginTop: 36 }}>
          <div className="wm" style={{ fontSize: "clamp(18px,3vw,28px)", color: C.gold, lineHeight: 1.6, marginBottom: 20 }}>
            "The audios don't motivate you.<br />They reprogram you at the level where motivation is no longer required."
          </div>
          <Btn onClick={() => onJoin("audio")}>Start your shift — €19.99/month</Btn>
        </div>
      </div>
      ...
      {/* HOW IT WORKS */}
      <div style={{ padding: "70px 20px", maxWidth: 1000, margin: "0 auto" }}>
        <div style={{ textAlign: "center", marginBottom: 48 }}>
          <div style={{ fontSize: 11, color: C.muted, letterSpacing: "0.25em", textTransform: "uppercase", marginBottom: 12 }}>How it works</div>
          <h2 className="wm" style={{ fontSize: "clamp(28px,5vw,48px)", color: C.text, marginBottom: 10 }}>Four steps. One reality shift.</h2>
          <p style={{ fontSize: 16, color: C.muted }}>Everything happens automatically. Your only job is to press play.</p>
        </div>
        <div className="step-flow" style={{ display: "flex", gap: 0, alignItems: "stretch" }}>
          {steps.map((s, i) => (
            <div key={i} style={{ display: "flex", flex: 1, alignItems: "stretch" }}>
              <div style={{ background: C.card, border: `1px solid ${C.border}`, flex: 1, padding: 24, borderLeft: i > 0 ? "none" : undefined, borderTopLeftRadius: i === 0 ? 16 : 0, borderBottomLeftRadius: i === 0 ? 16 : 0, borderTopRightRadius: i === 3 ? 16 : 0, borderBottomRightRadius: i === 3 ? 16 : 0, position: "relative", overflow: "hidden" }}>
                ...
                <div style={{ fontSize: 11, color: C.muted, fontWeight: 700, marginBottom: 12, fontFamily: "monospace", letterSpacing: "0.1em" }}>{s.n}</div>
                <div style={{ fontSize: 28, marginBottom: 12 }}>{s.icon}</div>
                <div style={{ fontSize: 16, fontWeight: 700, color: C.gold, marginBottom: 10 }}>{s.title}</div>
                <div style={{ fontSize: 14, color: C.muted, lineHeight: 1.7 }}>{s.body}</div>
              </div>
              {i < 3 && <div style={{ display: "flex", alignItems: "center", color: C.gold + "66", fontSize: 18, flexShrink: 0, padding: "0 4px" }}>→</div>}
            </div>
          ))}
        </div>
      </div>
      ...
      <div style={{ padding: "70px 20px", maxWidth: 1000, margin: "0 auto" }}>
        <div style={{ textAlign: "center", marginBottom: 48 }}>
          <div style={{ fontSize: 11, color: C.muted, letterSpacing: "0.25em", textTransform: "uppercase", marginBottom: 12 }}>The proof</div>
          <h2 className="wm" style={{ fontSize: "clamp(28px,5vw,48px)", color: C.text, marginBottom: 10 }}>This is what shifting looks like.</h2>
          <p style={{ fontSize: 16, color: C.muted }}>Real shifts. Not motivation. Not affirmations. Subconscious reprogramming.</p>
        </div>
        <div className="grid-3" style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 16 }}>
          ...
          <div style={{ background: C.card, border: `1px solid ${C.border}`, borderRadius: 16, overflow: "hidden" }}>
            <div style={{ padding: "12px 16px", borderBottom: `1px solid ${C.border}`, background: "#0d0508" }}>
              ...
              <div style={{ fontSize: 13, color: C.muted, marginTop: 2 }}>He's obsessed. Of course he is.</div>
            </div>
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr" }}>
              <div style={{ padding: 14, borderRight: `1px solid ${C.border}` }}>
                ...
                <div style={{ background: "#111", borderRadius: 10, padding: 10, fontSize: 11 }}>
                  ...
                  {["Hey are you there?", "Can we talk?", "Please respond"].map((m, i) => (
                    <div key={i} style={{ background: "#1a1a1a", borderRadius: 8, padding: "6px 8px", marginBottom: 4 }}>
                      <div style={{ color: "#555", fontSize: 11 }}>{m}</div>
                      <div style={{ color: "#2a2a2a", fontSize: 9, textAlign: "right", marginTop: 2 }}>✓ Delivered</div>
                    </div>
                  ))}
                  <div style={{ color: "#2a2a2a", fontSize: 10, textAlign: "center", marginTop: 8 }}>No reply · 8 days silence</div>
                </div>
                <div style={{ fontSize: 12, color: "#5a4040", marginTop: 8, fontStyle: "italic", lineHeight: 1.5 }}>Checking. Waiting. Desperate.</div>
              </div>
              <div style={{ padding: 14 }}>
                ...
                <div style={{ background: "#080f0a", borderRadius: 10, padding: 10, border: `1px solid #1a3a1a` }}>
                  ...
                  <div style={{ background: "#0f2a0f", borderRadius: 8, padding: "7px 9px", marginBottom: 4, border: `1px solid #1a3a1a` }}>
                    <div style={{ color: "#7ab07a", fontSize: 12 }}>Hey... been thinking about you constantly</div>
                    <div style={{ color: "#2a4a2a", fontSize: 9, marginTop: 3 }}>11:43pm · ✓✓ Read</div>
                  </div>
                  <div style={{ background: "#0f2a0f", borderRadius: 8, padding: "7px 9px", border: `1px solid #1a3a1a` }}>
                    <div style={{ color: "#6ab06a", fontSize: 12 }}>I miss you. Can I see you?</div>
                  </div>
                </div>
          ...
          <div style={{ background: C.card, border: `1px solid ${C.border}`, borderRadius: 16, overflow: "hidden" }}>
            <div style={{ padding: "12px 16px", borderBottom: `1px solid ${C.border}`, background: "#090d08" }}>
              <div style={{ fontSize: 12, color: "#6ab05a", fontWeight: 700, letterSpacing: "0.12em", textTransform: "uppercase" }}>Moneymaxxing</div>
              <div style={{ fontSize: 13, color: C.muted, marginTop: 2 }}>Money finds you first. Obviously.</div>
            </div>
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr" }}>
              <div style={{ padding: 14, borderRight: `1px solid ${C.border}` }}>
                <div style={{ fontSize: 10, color: "#5a3535", fontWeight: 700, letterSpacing: "0.2em", textTransform: "uppercase", marginBottom: 10 }}>Before</div>
                <div style={{ background: "#111", borderRadius: 10, padding: 10 }}>
                  <div style={{ fontSize: 10, color: "#3a3a3a", marginBottom: 6 }}>Bank · Current account</div>
                  <div style={{ fontSize: 22, fontWeight: 800, color: "#4a4a4a", marginBottom: 8 }}>€247.83</div>
                  {[["Rent", "−€900"], ["Bills", "−€180"], ["Food", "−€84"]].map(([k, v]) => (
                ...
                <div style={{ fontSize: 12, color: "#5a4040", marginTop: 8, fontStyle: "italic", lineHeight: 1.5 }}>Counting what's left.</div>
              </div>
              <div style={{ padding: 14 }}>
                <div style={{ fontSize: 10, color: C.gold, fontWeight: 700, letterSpacing: "0.2em", textTransform: "uppercase", marginBottom: 10 }}>After</div>
                <div style={{ background: "#080f08", borderRadius: 10, padding: 10, border: `1px solid #1a3a1a` }}>
                  <div style={{ fontSize: 10, color: "#4a8a4a", marginBottom: 6 }}>Bank · New transfer</div>
                  <div style={{ background: "#0a1f0a", borderRadius: 8, padding: "8px 10px", border: `1px solid #2a4a2a`, marginBottom: 6 }}>
                    ...
                    <div style={{ fontSize: 10, color: "#2a4a2a" }}>Unexpected. Source: ████</div>
                  </div>
                </div>
                <div style={{ fontSize: 12, color: C.text2, marginTop: 8, fontStyle: "italic", lineHeight: 1.5 }}>Exactly as asked. Of course.</div>
              </div>
            </div>
          </div>
          {/* Beauty */}
          <div style={{ background: C.card, border: `1px solid ${C.border}`, borderRadius: 16, overflow: "hidden" }}>
            <div style={{ padding: "12px 16px", borderBottom: `1px solid ${C.border}`, background: "#0d0c08" }}>
              ...
              <div style={{ fontSize: 13, color: C.muted, marginTop: 2 }}>Gorgeous is your default. Obviously.</div>
            </div>
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr" }}>
              <div style={{ padding: 14, borderRight: `1px solid ${C.border}` }}>
                <div style={{ fontSize: 10, color: "#5a3535", fontWeight: 700, letterSpacing: "0.2em", textTransform: "uppercase", marginBottom: 10 }}>Before</div>
                <div style={{ background: "#111", borderRadius: 10, padding: 12, textAlign: "center" }}>
                  <div style={{ width: 44, height: 44, borderRadius: "50%", background: "#1a1a1a", border: "2px solid #2a2a2a", margin: "0 auto 8px", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 20, filter: "grayscale(1) opacity(0.3)" }}>🪞</div>
                  <div style={{ fontSize: 11, color: "#4a3535", lineHeight: 1.5 }}>Picking apart every detail. Every morning. The same loop.</div>
                </div>
              ...
              <div style={{ padding: 14 }}>
                ...
                <div style={{ background: "#0a0900", borderRadius: 10, padding: 10, border: `1px solid ${C.dim}` }}>
                  <div style={{ fontSize: 10, color: C.muted, marginBottom: 8 }}>Messages · Today</div>
                  {[{ from: "Sarah", msg: "WHAT are you doing differently omg" }, { from: "Mia", msg: "Your skin is literally glowing?? Tell me everything" }].map((m, i) => (
                ...
                <div style={{ fontSize: 12, color: C.text2, marginTop: 8, fontStyle: "italic", lineHeight: 1.5 }}>They notice before you do.</div>
              </div>
            </div>
          </div>
        </div>
      ...
      <div style={{ padding: "70px 20px", maxWidth: 900, margin: "0 auto" }}>
        <div style={{ textAlign: "center", marginBottom: 36 }}>
          ...
          <h2 className="wm" style={{ fontSize: "clamp(28px,5vw,48px)", color: C.text, marginBottom: 10 }}>Every desire has its own frequency.</h2>
          <p style={{ fontSize: 16, color: C.muted }}>Ever-expanding. 4 new tracks every week. Never on YouTube. Never interrupted by ads.</p>
        </div>
        <div style={{ display: "flex", gap: 8, flexWrap: "wrap", justifyContent: "center", marginBottom: 20 }}>
          {cats.map(c => (
            <button key={c.id} onClick={() => setActiveCat(c.id)} style={{ padding: "8px 18px", borderRadius: 24, border: `1.5px solid ${activeCat === c.id ? c.color : C.border}`, background: activeCat === c.id ? c.color + "22" : "none", color: activeCat === c.id ? c.color : C.muted, fontSize: 14, fontWeight: 700, cursor: "pointer", minHeight: 40, transition: "all 0.2s" }}>
              {c.label}
            </button>
          ))}
        </div>
      ...
      {/* PROOFOS TEASER */}
      <div style={{ padding: "0 20px 70px", maxWidth: 900, margin: "0 auto" }}>
        <div style={{ position: "relative", background: "linear-gradient(135deg,#0a0700,#120808)", border: `2px solid ${C.gold}44`, borderRadius: 20, padding: 36, overflow: "hidden", cursor: "pointer" }} onClick={onProofOS}>
          <Rings count={3} />
          <div style={{ position: "relative", zIndex: 1 }}>
            ...
            <div className="wm" style={{ fontSize: "clamp(40px,8vw,72px)", lineHeight: 0.95, marginBottom: 12, background: `linear-gradient(90deg,${C.gold},${C.rose})`, WebkitBackgroundClip: "text", WebkitTextFillColor: "transparent" }}>ProofOS ✦</div>
            <div style={{ fontSize: 14, color: C.muted, letterSpacing: "0.15em", textTransform: "uppercase", marginBottom: 20 }}>The manifestation ledger</div>
            <p style={{ fontSize: 17, color: "#8a7a5a", lineHeight: 1.85, marginBottom: 28, maxWidth: 540 }}>
              Log your desire. Link it to the exact audio you listened to. Capture the signs — a screenshot, a voice note, a photo. Watch the pattern become undeniable. See exactly how many days it took from first listen to final proof.
            </p>
            <div style={{ display: "flex", gap: 20, marginBottom: 20, flexWrap: "wrap" }}>
              {[{ d: "He texted me", days: "14 days", track: "He's already on his way back" }, { d: "€10,000 arrived", days: "9 days", track: "Money is obsessed with me" }, { d: "Skin is visibly shifting", days: "21 days", track: "Gorgeous is my default" }].map((item, i) => (
                <div key={i} style={{ display: "flex", gap: 8, alignItems: "center" }}>
                  <div style={{ width: 8, height: 8, borderRadius: "50%", background: i < 2 ? C.green : C.gold }} />
                  <div>
                    <div style={{ fontSize: 13, color: C.text2, fontWeight: 600 }}>{item.d}</div>
                    <div style={{ fontSize: 11, color: C.dim }}>{item.track} · {item.days}</div>
                  </div>
                </div>
              ))}
            </div>
            <span style={{ fontSize: 15, color: C.gold, fontWeight: 700 }}>Find out more about ProofOS →</span>
          </div>
        </div>
      </div>

      {/* VIDEO PLACEHOLDER */}
      <div style={{ padding: "0 20px 70px", maxWidth: 700, margin: "0 auto" }}>
        <div style={{ background: C.card, border: `1px solid ${C.border}`, borderRadius: 18, padding: 36, textAlign: "center" }}>
          <div style={{ fontSize: 11, color: C.muted, letterSpacing: "0.2em", textTransform: "uppercase", marginBottom: 14 }}>From Reshma Oracle</div>
          <div style={{ width: 72, height: 72, borderRadius: "50%", background: `linear-gradient(135deg,${C.gold}22,${C.rose}22)`, border: `2px solid ${C.gold}44`, margin: "0 auto 16px", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 28 }}>🎧</div>
          <div className="wm" style={{ fontSize: 26, color: C.text, marginBottom: 8 }}>Hear it in her own voice.</div>
          <div style={{ fontSize: 15, color: C.muted }}>Voice message coming soon.</div>
        </div>
      </div>
      ...
      <div style={{ position: "relative", padding: "80px 20px", textAlign: "center", overflow: "hidden", borderTop: `1px solid ${C.border}` }}>
        ...
        <div style={{ position: "relative", zIndex: 1 }}>
          <div className="wm" style={{ fontSize: "clamp(28px,5vw,52px)", color: C.text, lineHeight: 1.2, marginBottom: 20 }}>
            The shift is already yours.<br />
            <span style={{ background: `linear-gradient(90deg,${C.gold},${C.rose})`, WebkitBackgroundClip: "text", WebkitTextFillColor: "transparent" }}>Of course it is.</span>
          </div>
          <Btn onClick={() => onJoin("audio")}>Start your shift — €19.99/month</Btn>
        </div>
      </div>
    ...
    <div style={{ minHeight: "100vh", background: C.black }} className="fade">
      ...
      <div style={{ position: "relative", padding: "72px 20px 56px", textAlign: "center", overflow: "hidden" }}>
        ...
        <div style={{ position: "relative", zIndex: 1, maxWidth: 620, margin: "0 auto" }}>
          ...
          <p style={{ fontSize: 16, color: "#8a7a5a", lineHeight: 1.85, marginBottom: 36 }}>ProofOS links every desire to the exact audio you listened to. It captures the signs. Records the proof. Shows you how many days from first listen to final result. The evidence becomes undeniable. Of course it does.</p>
          {!joined ? (
            <div style={{ maxWidth: 380, margin: "0 auto" }}>
              ...
              <div style={{ fontSize: 13, color: C.dim }}>Goddess Tier members get first access · No spam</div>
            </div>
          ) : (
            <div style={{ padding: "16px 24px", background: "#0a1a0a", border: `1px solid #2a4a2a`, borderRadius: 12, maxWidth: 380, margin: "0 auto", fontSize: 15, color: C.green, fontWeight: 600 }}>✓ You're on the list. First access when ProofOS launches.</div>
          )}
        </div>
      </div>
      {/* Steps */}
      <div style={{ padding: "0 20px 60px", maxWidth: 1000, margin: "0 auto" }}>
        ...
        <div className="step-flow" style={{ display: "flex", gap: 0 }}>
          {[
            { n: "01", icon: "✦", t: "Set your intention", b: "State exactly what you want. Present tense. Specific. This becomes your active proof thread." },
            { n: "02", icon: "🎧", t: "Listen with purpose", b: "Every track links automatically to your active threads. Every session dated and recorded." },
            { n: "03", icon: "◈", t: "Capture the proof", b: "Signs, synchronicities, results. Screenshot. Voice note. Photo receipt. Log them as they arrive." },
            { n: "04", icon: "✓", t: "Mark it manifested", b: "When it arrives — mark it. ProofOS shows days from first listen to final proof. It is done." },
          ].map((s, i) => (
      ...
      <div style={{ padding: "0 20px 60px", maxWidth: 640, margin: "0 auto" }}>
        <div style={{ background: C.card, border: `1px solid ${C.border}`, borderRadius: 16, overflow: "hidden" }}>
          ...
          <div style={{ padding: "12px 20px", borderBottom: `1px solid ${C.border2}`, background: "#080600" }}>
            ...
            <div style={{ fontSize: 14, fontWeight: 700, color: C.gold }}>🎧 Money is obsessed with me · 528hz · 9 sessions</div>
          </div>
          {[
            { day: "Day 1", icon: "🎧", label: "First listen", detail: "Night mode. Set intention. Pressed play.", c: C.muted },
            { day: "Day 3", icon: "🎙", label: "Voice note logged", detail: '"Felt something shift. Stopped needing it." — 0:47', c: C.text2 },
            { day: "Day 5", icon: "✦", label: "Sign noticed", detail: "Found €20 in an old jacket. Third coincidence this week.", c: C.gold },
            { day: "Day 9", icon: "✓", label: "Manifested — €5,000", detail: "Client I had forgotten. Paid in full. Of course.", c: C.green, final: true },
          ].map((e, i) => (
      ...
      <div style={{ padding: "0 20px 80px", textAlign: "center", maxWidth: 500, margin: "0 auto" }}>
        ...
        <p style={{ fontSize: 16, color: C.muted, marginBottom: 28, lineHeight: 1.75 }}>ProofOS is included in the Goddess Tier.<br />Start with the audio library now. The tracker unlocks when it launches.</p>
      ...
      <div style={{ width: "100%", maxWidth: 460 }}>
        ...
        {step === 2 && (
          <div className="fade">
            ...
            <div style={{ background: C.card, border: `1px solid ${C.gold}44`, borderRadius: 12, padding: 16, marginBottom: 16 }}>
              {(tier === "goddess" ? ["Full self hypnosis audio library", "4 new tracks/week", "ProofOS tracker coming soon ✦", "Early access + monthly ritual"] :
                tier === "founder" ? ["Full vault forever", "ProofOS for life", "1 GB evidence vault", "All future features", "Founder's seal ✦"] :
                  ["Full self hypnosis audio library", "4 new tracks/week", "Loop player + sleep timer", "Plays in background like Spotify", "No ads. Ever."]).map((f, i) => (
                    <div key={i} style={{ fontSize: 14, color: f.includes("✦") ? C.gold : C.text2, paddingLeft: 14, position: "relative", marginBottom: 7 }}>
                      <span style={{ position: "absolute", left: 0, color: C.gold, fontWeight: 700 }}>·</span>{f}
                    </div>
                  ))}
            </div>
        ...
        {step === 3 && (
          <div className="fade">
            ...
            <div style={{ background: C.card, border: `1px solid ${C.border}`, borderRadius: 12, padding: 16, marginBottom: 16, fontSize: 14, color: "#6a5a3a", lineHeight: 1.75 }}>
              <div style={{ fontSize: 11, color: C.muted, marginBottom: 8, letterSpacing: "0.15em", textTransform: "uppercase", fontWeight: 700 }}>Stripe secure checkout</div>
              Redirected to Stripe's secure payment page.<br />No card data stored by us · SSL encrypted<br />No refunds after 14 days from payment date
            </div>
        ...
        {step === 4 && (
          <div style={{ textAlign: "center" }} className="fade">
            ...
            <div style={{ fontSize: 16, color: C.muted, marginBottom: 36, lineHeight: 1.8 }}>
              Your portal is ready. Currently right now.<br />
              {tier === "goddess" && "ProofOS tracker activates when it launches. Of course it will."}
...
function Portal({ userTier, onSignOut, onUpgrade }) {
  ...
  const catColors = { love: C.rose, money: "#b87a20", beauty: C.gold, sleep: "#5a7ab0", dna: "#8a6ad0", identity: "#b07060", all: C.gold };
  ...
  const tracks = [
    ...
    { id: 4, title: "I've always been the prize", cat: "identity", tag: "Lifemaxxing", freq: "LOA", type: "Subliminal", dur: "30:00", tier: "audio" },
    ...
    { id: 8, title: "Reality rearranges for me", cat: "identity", tag: "Lifemaxxing", freq: "963hz", type: "Frequency", dur: "33:00", tier: "goddess" },
  ...
  const cats = [{ id: "all", label: "All" }, { id: "love", label: "Lovemaxxing" }, { id: "money", label: "Moneymaxxing" }, { id: "beauty", label: "Beautymaxxing" }, { id: "sleep", label: "Sleep" }, { id: "dna", label: "DNA" }, { id: "identity", label: "Lifemaxxing" }];
  ...
  const signSuggestions = ["Saw a number sequence", "Dreamed about it", "Someone mentioned it randomly", "Found money unexpectedly", "Received a surprise message", "Felt calm certainty", "Overheard a conversation about it"];
  ...
  return (
    <div style={{ minHeight: "100vh", background: C.black }} className="fade">
      ...
      <div className="desk-tabs" style={{ display: "flex", borderBottom: `1px solid ${C.border}`, background: "#050300", position: "sticky", top: 56, zIndex: 49 }}>
        {tabs.map(t => (
          <button key={t.id} onClick={() => setTab(t.id)} style={{ flex: 1, padding: "13px 8px", background: "none", border: "none", borderBottom: `2px solid ${tab === t.id ? C.gold : "transparent"}`, color: tab === t.id ? C.gold : C.muted, fontSize: 13, fontWeight: 700, letterSpacing: "0.1em", textTransform: "uppercase", cursor: "pointer" }}>{t.label}</button>
        ))}
      </div>
      {/* Stats dashboard */}
      <div style={{ display: "grid", gridTemplateColumns: "repeat(4,1fr)", borderBottom: `1px solid ${C.border}`, background: "#060400" }}>
        {[
          { v: tracks.filter(t => canPlay(t)).length, l: "Tracks", c: C.text2, sub: "in your vault" },
          { v: desires.length, l: "Intentions", c: C.gold, sub: "set" },
          { v: manifested, l: "Manifested", c: C.rose, sub: `${inProgress} in progress` },
          { v: "14d", l: "Streak", c: C.green, sub: "listening daily" },
        ].map((s, i) => (
      ...
      {tab === "tracker" && userTier === "goddess" && (
        <div style={{ background: "#060400", padding: "8px 16px", borderBottom: `1px solid ${C.border}` }}>
          <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 4 }}>
            <span style={{ fontSize: 11, color: C.muted, fontWeight: 600 }}>Manifestation rate</span>
            <span style={{ fontSize: 11, color: C.gold, fontWeight: 700 }}>{manifested}/{desires.length} · {Math.round((manifested / desires.length) * 100)}%</span>
          </div>
          <div style={{ height: 4, background: C.border, borderRadius: 2 }}>
            <div style={{ width: `${(manifested / desires.length) * 100}%`, height: "100%", background: `linear-gradient(90deg,${C.gold},${C.rose})`, borderRadius: 2, transition: "width 0.6s" }} />
          </div>
        </div>
      )}
      ...
      <div style={{ display: "flex", minHeight: "calc(100vh - 140px)" }}>
        ...
        <div className="sidebar" style={{ width: 196, borderRight: `1px solid ${C.border}`, background: "#040200", flexShrink: 0, position: "sticky", top: 112, height: "calc(100vh - 112px)", overflowY: "auto", padding: "14px 8px" }}>
          {cats.map(c => (
            <button key={c.id} onClick={() => { setActiveCat(c.id); setTab("audios"); }} style={{ width: "100%", display: "flex", alignItems: "center", gap: 10, padding: "9px 12px", borderRadius: 10, border: "none", background: activeCat === c.id && tab === "audios" ? (catColors[c.id] || C.gold) + "18" : "none", cursor: "pointer", marginBottom: 3, textAlign: "left", transition: "all 0.15s" }}>
                  ...
                  return (
                    <div key={t.id} onClick={() => can && setPlaying(isP ? null : t.id)}
                      ...
                      <div style={{ width: 36, height: 36, borderRadius: "50%", background: isP ? color + "22" : "#080600", border: `1.5px solid ${isP ? color : C.border}`, display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0 }}>
                        {!can ? <span>🔒</span> : isP ? <div className="sw" style={{ height: 20, gap: 2 }}><span style={{ height: 6 }} /><span style={{ height: 12 }} /><span style={{ height: 18 }} /><span style={{ height: 10 }} /><span style={{ height: 14 }} /></div> : <span style={{ fontSize: 13, color: C.muted }}>▶</span>}
                      </div>
                    </div>
                  );
                })}
            ...
            <div className="fade">
              {userTier !== "goddess" ? (
                <div style={{ textAlign: "center", padding: "60px 20px" }}>
                  ...
                  <p style={{ fontSize: 16, color: C.muted, marginBottom: 28, lineHeight: 1.75 }}>Log your desires. Link them to your listening.<br />Watch the proof accumulate in real time.</p>
                ...
                <div>
                  ...
                  <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 10, marginBottom: 16 }}>
                    {[
                      { v: manifested, l: "Manifested", c: C.rose, sub: "desires completed" },
                      { v: inProgress, l: "In progress", c: C.gold, sub: "currently active" },
                      { v: desires.reduce((a, d) => a + d.signs.length, 0), l: "Signs logged", c: C.green, sub: "total synchronicities" },
                    ].map((s, i) => (
                  ...
                  <div style={{ background: C.card, border: `1px solid ${C.border}`, borderRadius: 12, padding: 16, marginBottom: 14 }}>
                    ...
                    <input value={newDesire} onChange={e => setNewDesire(e.target.value)} placeholder="State it in present tense — I am, I have, I receive, currently right now..." style={{ marginBottom: 10 }} onKeyDown={e => e.key === "Enter" && addDesire()} />
                        ...
                        <div key={d.id} style={{ background: C.card, border: `1px solid ${d.status === "manifested" ? "#2a4a2a" : C.border}`, borderRadius: 12, overflow: "hidden" }}>
                          <div style={{ padding: "14px 16px", cursor: "pointer", display: "flex", justifyContent: "space-between", alignItems: "flex-start", gap: 10 }} onClick={() => setExpanded(expanded === d.id ? null : d.id)}>
                            <div style={{ flex: 1, minWidth: 0 }}>
                              ...
                              {d.track && <div style={{ display: "flex", alignItems: "center", gap: 6, marginBottom: 4 }}><span style={{ fontSize: 13 }}>🎧</span><span style={{ fontSize: 13, color: color, fontWeight: 600 }}>{d.track}</span>{d.days > 0 && <span style={{ fontSize: 12, color: C.dim }}>· {d.days} days</span>}</div>}
                              {d.signs.length > 0 && <div style={{ fontSize: 12, color: C.muted }}>✦ {d.signs.length} sign{d.signs.length !== 1 ? "s" : ""} logged</div>}
                            </div>
                            <div style={{ display: "flex", flexDirection: "column", alignItems: "flex-end", gap: 8, flexShrink: 0 }}>
                              ...
                              <span style={{ fontSize: 11, color: C.dim }}>{expanded === d.id ? "▲" : "▼ signs"}</span>
                            </div>
                          </div>
                          {expanded === d.id && (
                            <div style={{ borderTop: `1px solid ${C.border2}`, padding: "14px 16px", background: "#060400" }}>
                              <div style={{ fontSize: 11, color: C.muted, letterSpacing: "0.15em", textTransform: "uppercase", fontWeight: 700, marginBottom: 10 }}>Signs & synchronicities</div>
                              {d.signs.length === 0 && <div style={{ fontSize: 14, color: C.dim, marginBottom: 12 }}>No signs logged yet. The universe is already moving.</div>}
                              {d.signs.map((s, i) => (
                                <div key={i} style={{ fontSize: 14, color: C.muted, paddingLeft: 14, position: "relative", marginBottom: 7 }}>
                                ...
                                <div style={{ marginTop: 12 }}>
                                  ...
                                  <div style={{ display: "flex", flexWrap: "wrap", gap: 6, marginBottom: 12 }}>
                                    {signSuggestions.slice(0, 4).map((s, i) => (
                                      <button key={i} onClick={() => addSign(d.id, s)} style={{ padding: "5px 12px", background: "none", border: `1px solid ${C.border}`, borderRadius: 20, color: C.muted, fontSize: 12, cursor: "pointer" }}>{s}</button>
                                    ))}
                                  </div>
          ...
          {tab === "account" && (
            <div className="fade" style={{ display: "flex", flexDirection: "column", gap: 12 }}>
              <div style={{ background: C.card, border: `1px solid ${C.border}`, borderRadius: 14, padding: 20 }}>
                ...
                <div style={{ fontSize: 16, fontWeight: 700, color: C.text, marginBottom: 14 }}>member@reshmaoracle.com</div>
                <div style={{ fontSize: 13, color: C.muted, marginBottom: 3 }}>Current plan</div>
                <div style={{ fontSize: 16, fontWeight: 700, color: userTier === "goddess" ? C.gold : C.text2 }}>{userTier === "goddess" ? "Goddess Tier · €33/month" : "Audio Tier · €19.99/month"}</div>
              </div>
              {userTier === "audio" && (
                <div onClick={onUpgrade} style={{ background: "#0a0500", border: `1.5px solid ${C.gold}44`, borderRadius: 14, padding: 20, cursor: "pointer" }}>
                  <div style={{ fontSize: 11, color: C.rose, letterSpacing: "0.15em", textTransform: "uppercase", fontWeight: 700, marginBottom: 8 }}>Upgrade available</div>
                  <div style={{ fontSize: 16, fontWeight: 700, color: C.text, marginBottom: 4 }}>Goddess Tier — €33/month</div>
                  <div style={{ fontSize: 14, color: C.muted }}>Unlock ProofOS tracker + early access →</div>
                </div>
              )}
              <div style={{ background: C.card, border: `1px solid ${C.border}`, borderRadius: 14, padding: 20 }}>
                <div style={{ fontSize: 11, color: C.muted, letterSpacing: "0.15em", textTransform: "uppercase", fontWeight: 700, marginBottom: 14 }}>Subscription</div>
                <div style={{ fontSize: 15, color: "#7a6a4a", lineHeight: 1.9, marginBottom: 14 }}>Next billing: 29 July 2026<br />No refunds after 14 days<br />Cancel before renewal</div>
                <Btn small outline onClick={() => {}}>Manage billing ↗</Btn>
              </div>
              <div style={{ background: C.card, border: `1px solid ${C.border}`, borderRadius: 14, padding: 20 }}>
                <div style={{ fontSize: 11, color: C.muted, letterSpacing: "0.15em", textTransform: "uppercase", fontWeight: 700, marginBottom: 14 }}>Access</div>
                <div style={{ fontSize: 15, color: "#7a6a4a", lineHeight: 1.9 }}>Any browser · iPhone: tap Share → "Add to Home Screen"<br />Plays in background like Spotify when screen is locked<br />No App Store download needed</div>
              </div>
              <div style={{ display: "flex", gap: 12 }}>
                {[["YouTube", "https://www.youtube.com/@Reshma.Oracle"], ["Instagram", "https://www.instagram.com/reshma.oracle/"]].map(([l, u]) => (
                  <a key={l} href={u} target="_blank" rel="noopener noreferrer" style={{ flex: 1, padding: 14, background: C.card, border: `1px solid ${C.border}`, borderRadius: 10, color: C.muted, fontSize: 14, textAlign: "center", textDecoration: "none" }}>{l} →</a>
                ))}
              </div>
      ...
      <div className="mob-nav" style={{ position: "fixed", bottom: 0, left: 0, right: 0, background: "#050300", borderTop: `1px solid ${C.border}`, zIndex: 200, paddingBottom: "env(safe-area-inset-bottom,8px)" }}>
        {tabs.map(t => (
          <button key={t.id} onClick={() => setTab(t.id)} style={{ flex: 1, padding: "10px 8px", background: "none", border: "none", color: tab === t.id ? C.gold : C.muted, fontSize: 10, fontWeight: 700, cursor: "pointer", display: "flex", flexDirection: "column", alignItems: "center", gap: 4, minHeight: 56, letterSpacing: "0.08em", textTransform: "uppercase" }}>
