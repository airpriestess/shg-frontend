      ...
      <div style={{ marginTop: `calc(${isMobile ? "98px" : "102px"} + env(safe-area-inset-top,0px))` }}>
        ...
        <div style={{ background:"#000", padding: isMobile?"48px 20px":"64px 48px", borderTop:"1px solid rgba(232,184,112,0.08)" }}>
          <div style={{ maxWidth:1100, margin:"0 auto", width:"100%", display:"flex", flexDirection: isMobile?"column":"row", alignItems:"center", gap: isMobile?40:64 }}>
            ...
            <div className="hscroll-ok" style={{ flex:"0 0 auto", display:"flex", gap: isMobile?12:18, overflowX:"auto", maxWidth:"100%", padding:"8px 4px 16px", scrollSnapType:"x mandatory" }}>
              {[["home","Home"],["library","Library"],["player","Now playing"]].map(([k,label])=>(
                <div key={k} style={{ flex:"0 0 auto", scrollSnapAlign:"center", textAlign:"center" }}>
                  <div style={{ width: isMobile?200:190, borderRadius:30, padding:6, background:"linear-gradient(135deg,#F5E0A0,#E8B870,#BFA5D8,#2CB7A7,#167A6B)", boxShadow:"0 0 50px rgba(44,183,167,0.18)" }}>
                    <img src={`/mockups/app-${k}.webp`} alt={`${label} screen of the Self Hypnosis Goddess app`} loading="lazy" style={{ display:"block", width:"100%", borderRadius:24, background:"#000" }}/>
                  </div>
                  <div style={{ marginTop:10, fontFamily:"'Jost',sans-serif", fontWeight:300, fontSize:12, letterSpacing:"0.18em", textTransform:"uppercase", color:"#F2ECE4" }}>{label}</div>
                </div>
              ))}
            </div>
